			...
			<ul class="topnav-ul" style="float: left;">
				...
				<li class="topnav-li" style="position: relative;">
					...
					<div class="dropdown">
						...
						<a href="#" class="nav-page-text" style="font-size: 16px;  color: gray"><i class="fas fa-user"></i>
					<span style="padding: 10px">Edit Profile</span></a>
					</div>
				</li>
...
<div class="sizer" style="background-color: #f1f1f1">
	<div class="top-image-holder">
		<div class="top-image-text-holder">
			<p class="top-image-text">
				Manage and Find your Friends here
			</p>
        ...
        <%
              int action = Integer.parseInt(session.getAttribute("action").toString());
              int btntemp = action%10;
              int UIDtemp = action/10;
              String fNameT="";
              String lNameT="";
              try{
                        Class .forName("com.mysql.jdbc.Driver");
                        Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/matchmaker","root","");  //databse connectivity                                                                                               
                        Statement stmt=(Statement) con.createStatement();
                        ResultSet rs = stmt.executeQuery("select * from user_info where id like '"+UIDtemp+"'");
                        rs.next();
                        fNameT = rs.getString("firstName");
                        lNameT = rs.getString("lastName");
                        
              }catch(Exception v){out.print("--catch---");}
              if(btntemp==1)
              {
        %>
        
	<div class="actionBtn-success" ">
		<p class="action-success"><%out.print("Request sent to "+fNameT+" "+lNameT);%></p>
	</div>
        <%
        }
         else if(btntemp==4)
	...
	<div class="actionBtn-success">
		<p class="action-success"><%out.print("Now you are friend with "+fNameT+" "+lNameT);%></p>
	</div>
        <%
        }
          else if(btntemp==5)
	...
	<div class="actionBtn-success">
		<p class="action-success" style="background:linear-gradient(to right, red , orange)"><%out.print("Now you are not friend with "+fNameT+" "+lNameT);%></p>
	</div>
        <%
        }
        %>
                    ...
                    <%
                     ...
                     try 
                       ...
                       count = 0;
                        ...
                        Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/matchmaker","root","");  //databse connectivity                                                                                               
                        Statement stmt=(Statement) con.createStatement();
                        ResultSet rs = stmt.executeQuery("select * from look_for where email like '"+session.getAttribute("emailId")+"'");
                        rs.next();
                        ...
                        String ageFrom = rs.getString("ageFrom");
                        String ageTo = rs.getString("ageTo");
                        ...
                        builder.append("select * from user_info where "
                                + "email like '"+session.getAttribute("emailId")+"'");
                        
                        if(!gender.equalsIgnoreCase("Any")){
                            builder.append(" and gender like '"+gender+"'");
                        }if(!caste.equalsIgnoreCase("Any")){
                            builder.append(" and caste like '"+caste+"'");
                        }if(!dosh.equalsIgnoreCase("Any")){
                            builder.append(" and dosh like '"+dosh+"'");
                        }if(!maritalStatus.equalsIgnoreCase("Any")){
                            builder.append(" and maritalStatus like '"+maritalStatus+"'");
                        }if(!religion.equalsIgnoreCase("Any")){
                            builder.append(" and religion like '"+religion+"'");
                        }if(!familyStatus.equalsIgnoreCase("Any")){
                            builder.append(" and familyStatus like '"+familyStatus+"'");
                        }if(!familyType.equalsIgnoreCase("Any")){
                            builder.append(" and familyType like '"+familyType+"'");
		...
		<p class="load-more">Friends requests:</p>
	</div>
                    <%  int z=0;
                        while(loop<250)
                        {
                           ...
                           loop = loop +1;
                            try
                            {// ----while-loop-try
                               rs = stmt.executeQuery("select * from friends where iUID like '"+list+"' and rUID like '"+UID+"' and sender like 1 and accepter like 0");
                               rs.next();
                                {
                                    int accepter =Integer.parseInt(rs.getString("accepter"));
                                    {   rs = stmt.executeQuery("select * from user_info where id like "+list+"");
                                        rs.next();
                                        String fName = rs.getString("firstName");
                                        int requestBtn = list * 10 + 4;
                                        int blockBtn = list * 10 + 2;
                                        String lName = rs.getString("lastName");
                                        String icon = rs.getString("icon");
                                        z=z+1;
                                         String emailTemp = rs.getString("email");
                                        int visit = UID *1000 + list;     
                            %>
			...
			<div class="grid">
				...
				<div class="friend-name-hold">
					<p class="friend-name"><%out.print(""+fName+" "+lName);%></p>
					<p class="friend-name-mutual"><%out.print(emailTemp);%></p>
				</div>
				<div class="friend-action-btn-holder">
					...
					<div class="action-btn"  style=" background-color: mediumseagreen">
						<i class="fas fa-user-plus action-icon"></i>
					</div>
                                        </a>
                                       
                                        <a href="profilePage2.jsp?action1Btn=<%out.print(visit);%>" style="cursor:pointer" title="View Profile">	
						...
						<i class="fas fa-eye action-icon"></i>
                           ...
                           <%       
                            ...
                            { 
                              ...
                              if(z==0)
                              {
                               %><h1 style=" text-align: center;color: gray; margin-top: 10px; margin-bottom: 20px;font-family: calibri"><% out.print("No requests!");%></h1><%
                  ...
                  &nbsp;<br>
                   &nbsp;<br>
                    &nbsp;<br>
                        <div class="load-more-hold">
		<p class="load-more">Create Friends :</p>
	</div>
                    <%  int list = 0;
                        try 
                        {    //-----try-1  
                        ...
                        while(loop<15)
                        {
                            ...
                            try
                            {// ----while-loop-try
                                rs = stmt.executeQuery("select * from friends where iUID like '"+UID+"' and rUID like '"+list+"'");
                               rs.next();
                                {
                                    int accepter =Integer.parseInt(rs.getString("accepter"));
	...
	<div class="load-more-hold">
	<a  href="findFriend2.jsp?LoadBtn=<%out.print(list);%>" style="cursor:pointer" title="Find more friends">	
						
            <p class="load-more">Load More</p></a>
	</div>
</div><!--sizer-->
